...
var _ = Describe("PackedEncoder", func() {
	...
	DescribeTable("types",
		func(data any, expected []byte, expectedAlt ...[]byte) {
			Expect(encoder.Encode(data)).To(Succeed())
			if len(expectedAlt) == 0 {
				Expect(buffer.Bytes()).To(Equal(expected))
			} else {
				Expect(buffer.Bytes()).To(SatisfyAny(
					Equal(expected),
					Equal(expectedAlt[0]),
				))
			}
		...
		Entry("CustomBytes", // fast track
			CustomBytes{0x31, 0xFA, 0x45, 0xAC, 0x5F, 0x21},
			seq(
				0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // length
				0x31, 0xFA, 0x45, 0xAC, 0x5F, 0x21, // items
			),
		...
		Entry("CustomSlice",
			CustomSlice([]uint16{0x31FA, 0x45AC, 0x5F21}),
			seq(
				0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // length
				0xFA, 0x31, 0xAC, 0x45, 0x21, 0x5F, // items
			),
